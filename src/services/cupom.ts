...
export class MockPrinter implements Printer {
  ...
  qr(_content: string, _size?: number): void {
    ...
    this.lines.push(
      "██ SIMULA   ██".padStart((this.colunas + 14) / 2).padEnd(this.colunas),
    );
    this.lines.push(
      "██ QR CODE  ██".padStart((this.colunas + 14) / 2).padEnd(this.colunas),
    );
    this.lines.push(
      "██████████████".padStart((this.colunas + 14) / 2).padEnd(this.colunas),
    );
...
export function renderizarCupom(
  ...
  p.text(
    `${pad}${"QTD".padEnd(lQtd)}${"ITEM".padEnd(lNomeMax)}${"TOTAL".padStart(lPreco)}\n`,
  );
  p.text(`${pad}${"-".repeat(util)}\n`);
  ...
  if (chavePix && totalGeral > 0) {
    const pixStr = gerarPixCopiaECola(
      chavePix,
      totalGeral,
      nomeLoja,
      cidadeLoja,
...
export function gerarPreview(
  ...
  if (isPremium) {
    if (loja.logo_url) {
      // MockPrinter handles imageUrl directly for the preview
      lojaParaRender.logo_url = loja.logo_url;
    }
  } else {
