...
export async function POST(req: NextRequest) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      console.log(
        `✅ [Webhook] Checkout concluído. Loja: ${lojaId}, Sub: ${subscriptionId}`,
      );
      ...
      if (lojaId) {
        ...
        if (subscriptionId) {
          try {
            ...
            console.error(
              "⚠️ [Webhook] Erro ao processar data da assinatura:",
              e,
            );
          }
        }
        ...
        if (error) {
          console.error(
            `❌ [Webhook] Erro ao atualizar loja por ID (${lojaId}):`,
            error.message,
          );
        }
      } else if (customerId) {
        ...
        console.warn(
          "⚠️ [Webhook] Metadata loja_id ausente. Usando fallback por customer_id.",
        );
        const { error } = await supabase
          .from("lojas")
          .update({
        ...
        if (error) {
          console.error(
            "❌ [Webhook] Erro no fallback por customer_id:",
            error.message,
          );
        }
      }
    ...
    case "customer.subscription.updated": {
      ...
      if (error) {
        console.error(
          "❌ [Webhook] Erro ao atualizar status da assinatura:",
          error.message,
        );
      }
      break;
    ...
    case "customer.subscription.deleted": {
      ...
      if (error) {
        console.error(
          "❌ [Webhook] Erro ao cancelar assinatura:",
          error.message,
        );
      }
      break;
